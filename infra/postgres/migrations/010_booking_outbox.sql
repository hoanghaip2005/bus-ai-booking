...
CREATE TABLE IF NOT EXISTS booking.outbox_events (
  ...
  CONSTRAINT booking_outbox_event_destination_key UNIQUE (event_id, destination),
  CHECK (published_at IS NULL OR dead_lettered_at IS NULL)
);

CREATE INDEX IF NOT EXISTS booking_outbox_dispatch_idx
ON booking.outbox_events (available_at, occurred_at)
WHERE published_at IS NULL AND dead_lettered_at IS NULL;

CREATE INDEX IF NOT EXISTS booking_outbox_aggregate_idx
ON booking.outbox_events (aggregate_id, occurred_at);

INSERT INTO platform.schema_migrations (version)
VALUES ('010_booking_outbox')
