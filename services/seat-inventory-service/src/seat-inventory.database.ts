...
@Injectable()
export class SeatInventoryDatabase implements OnModuleDestroy {
  private readonly pool = new Pool({
    connectionString:
      process.env.SEAT_INVENTORY_DATABASE_URL ??
      'postgresql://bus:bus_local_password@localhost:5432/bus_platform',
    max: Number(process.env.SEAT_INVENTORY_DATABASE_POOL_SIZE ?? 10),
    application_name: 'seat-inventory-service',
  });
  ...
  constructor() {
    this.pool.on('error', () => {
      logEvent({
        ...
        level: 'error',
        event: 'seat-inventory.postgresql.idle-client-error',
