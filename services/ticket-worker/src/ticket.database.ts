...
@Injectable()
export class TicketDatabase implements OnModuleDestroy {
  private readonly pool = new Pool({
    connectionString:
      process.env.TICKET_DATABASE_URL ??
      'postgresql://bus:bus_local_password@localhost:5432/bus_platform',
    max: Number(process.env.TICKET_DATABASE_POOL_SIZE ?? 10),
    application_name: 'ticket-worker',
  });
  ...
  constructor() {
    this.pool.on('error', () => {
      logEvent({
        ...
        level: 'error',
        event: 'ticket.postgresql.idle-client-error',
